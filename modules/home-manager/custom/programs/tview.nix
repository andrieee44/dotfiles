  ...
  custom.programs.tview = {
    package =
      let
        src = pkgs.fetchFromGitHub {
          owner = "andrieee44";
          repo = "tview";
          rev = "fcf671a8f6832db14024878219f4cb496d0061e9";
          hash = "sha256-VVSLBLGGDHbP7Aw4J0DIptI0cKU0CZggxuLKHacxEcc=";
        };
      in
      pkgs.buildGoModule {
        inherit src;
        name = "tview";
        vendorHash = "sha256-6HY0TmmKdm1RIN8xfHUEq6n829XkwnxhXLjls/YaCNk=";
    ...
    settings =
      ...
      in
      {
        ...
        "application/pdf" = [
          ''${pkgs.poppler-utils}/bin/pdftoppm -jpeg -f 1 -singlefile -- "$TVIEW_FILE" | ${pkgs.chafa}/bin/chafa -s "${"\${TVIEW_COLUMNS}"}x${"\${TVIEW_ROWS}"}" $([ "${"\${XDG_SESSION_TYPE:-}"}" = "tty" ] || ${pkgs.toybox}/bin/printf -- "-f sixels")''
        ];
